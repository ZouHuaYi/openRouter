...
在页面上配置服务商、模型列表、环境变量，并一键启动/停止网关。前端为 **Vue 3 + Vite + UnoCSS**，源码在 `electron/ui-src`，构建输出到 `electron/ui`。
...
- **模型列表**：增删改后端列表，拖拽调整优先级；**限流/停用**的模型会单独标记，可设解封时间（1 小时 / 1 天 / 1 周 / 1 月），到期后自动重新参与轮询；收到 429 时网关也会自动为该后端写入 1 小时 cooldown。
